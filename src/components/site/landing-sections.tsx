    ...
    <section key={section.id} className="mx-auto grid max-w-7xl gap-8 px-4 pt-10 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:pt-16">
      <div className="space-y-6">
        {section.payload.content.eyebrow ? (
          <div className="inline-flex rounded-full bg-white/75 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.24em] text-[var(--brand-primary)] shadow-sm">
            {section.payload.content.eyebrow}
          </div>
        ) : null}
        ...
        <div className="grid gap-3 sm:flex sm:flex-wrap">
          {section.payload.cta.label && section.payload.cta.href ? (
            <Link href={section.payload.cta.href} className="inline-flex min-h-12 items-center justify-center rounded-full bg-stone-950 px-6 text-sm font-semibold text-white transition hover:-translate-y-0.5">
        ...
        {stats.length ? (
          <div className="grid gap-4 sm:grid-cols-3">
            {stats.map((item) => (
              <div key={item.id ?? item.label ?? item.value} className="rounded-[1.75rem] border border-stone-200/70 bg-white/75 p-5 shadow-sm backdrop-blur">
      ...
      <div className="relative overflow-hidden rounded-[2.25rem] border border-stone-200/70 bg-[linear-gradient(160deg,#fff_0%,#f6e7d7_40%,#d8b28f_100%)] p-6 shadow-[var(--card-shadow)] sm:p-8">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.8),transparent_38%)]" />
        <div className="relative space-y-6">
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm leading-7 text-stone-700 sm:text-base">Premium vitrin ile yalnizca guzel gorunen degil, kolay tasinan ve sifir hosting butcesine uyabilen bir deneyim hedefleniyor.</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section key={section.id} className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        {categories.map((category) => (
          <Link key={category.id} href={`/urunler?kategori=${category.slug}`} className="rounded-[1.75rem] border border-stone-200/70 bg-white/80 p-5 shadow-sm transition hover:-translate-y-1">
...
function renderFeaturedProducts(section: LandingSectionRecord, featuredProducts: ProductRecord[]) {
  ...
  return (
    <section key={section.id} className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <SectionHeading eyebrow={section.payload.content.eyebrow} title={section.payload.content.title ?? "One Cikan Urunler"} description={section.payload.content.body} />
...
function renderCards(section: LandingSectionRecord, tone: "plain" | "steps" = "plain") {
      ...
      <section key={section.id} className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-3">
          {section.payload.items.map((item, index) => (
            <article key={item.id ?? `${section.id}-${index}`} className="rounded-[2rem] border border-stone-200/80 bg-white/80 p-6 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-[0.25em] text-[var(--brand-primary)]">{item.label ?? `Adim ${index + 1}`}</p>
  ...
  return (
    <section key={section.id} className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div className="grid gap-8 rounded-[2.25rem] border border-stone-200/70 bg-white/75 p-5 shadow-[var(--card-shadow)] sm:p-8 lg:grid-cols-[0.95fr_1.05fr]">
        ...
        <div className={`grid gap-4 ${section.payload.items.length > 3 ? "sm:grid-cols-2 xl:grid-cols-4" : "sm:grid-cols-3"}`}>
          {section.payload.items.map((item, index) => (
            <article key={item.id ?? `${section.id}-${index}`} className="rounded-[1.75rem] border border-stone-200/80 bg-stone-50 p-5">
  ...
  return (
    <section key={section.id} className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div className="grid gap-6 lg:grid-cols-[1fr_0.95fr] lg:items-start">
        ...
        <div className="rounded-[2rem] border border-stone-200/80 bg-[linear-gradient(160deg,#fff_0%,#f6e7d7_45%,#d8b28f_100%)] p-6 shadow-[var(--card-shadow)]">
          ...
          <p className="mt-4 text-base leading-8 text-stone-700">{section.payload.content.highlight ?? "Landing duzeni sabit data ile yonetilir."}</p>
        </div>
      </div>
      {section.payload.items.length ? (
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {section.payload.items.map((item, index) => (
            <article key={item.id ?? `${section.id}-${index}`} className="rounded-[1.75rem] border border-stone-200/80 bg-white/85 p-5 shadow-sm">
...
function renderFaq(section: LandingSectionRecord) {
  ...
  return (
    <section key={section.id} className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div className="rounded-[2rem] border border-stone-200/80 bg-white/85 p-5 shadow-sm sm:p-6">
        <SectionHeading eyebrow={section.payload.content.eyebrow} title={section.payload.content.title ?? "Sik Sorulanlar"} description={section.payload.content.body} />
        <div className="mt-8 space-y-4">
          {section.payload.items.map((item, index) => (
            <article key={item.id ?? `${section.id}-${index}`} className="rounded-[1.5rem] border border-stone-200 bg-stone-50 px-4 py-4">
...
function renderCtaBand(section: LandingSectionRecord) {
  return (
    <section key={section.id} className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div className="rounded-[2.25rem] bg-stone-950 px-6 py-8 text-white shadow-[var(--card-shadow)] sm:px-8 sm:py-10">
...
function renderSocialStrip(section: LandingSectionRecord, socialLinks: SocialLink[]) {
  ...
  return (
    <section key={section.id} className="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
      <SectionHeading eyebrow={section.payload.content.eyebrow} title={section.payload.content.title ?? "Baglantilar"} description={section.payload.content.body} />
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {socialLinks.map((link) => (
          <a key={link.id} href={link.url} target="_blank" rel="noreferrer" className="rounded-[1.75rem] border border-stone-200/80 bg-white/85 p-5 shadow-sm transition hover:-translate-y-0.5">
            ...
            <p className="mt-2 text-sm text-stone-600">{link.username ? `@${link.username}` : "Baglantiyi ac"}</p>
          </a>
        ))}
      </div>
    </section>
